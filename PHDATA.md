...
Sound Realty has provided a Python script (`scripts/create_model.py`) which trains the
basic model.
...
in `scripts/create_model.py` and try to figure out how well the model will generalize
to new data.  Has the model appropriately fit the dataset?

## Docker Deployment

The application can be run using Docker Compose, which will start all required services:
- FastAPI server
...
- Model files generated (run `python scripts/create_model.py` first)

### Quick Start

1. **Generate the model** (if not already done):
   ```bash
   python scripts/create_model.py
   ```
...
- `./docker-run.sh build` - Rebuild Docker images
...
- `./docker-run.sh clean` - Remove everything (including volumes)
...
This will start 3 worker instances to process prediction jobs in parallel.
